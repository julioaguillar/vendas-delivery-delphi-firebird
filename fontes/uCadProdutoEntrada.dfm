inherited frmCadProdutoEntrada: TfrmCadProdutoEntrada
  BorderIcons = [biSystemMenu]
  Caption = 'Lan'#231'amento - Entrada de Produto/Mercadoria'
  ClientHeight = 154
  ClientWidth = 588
  Position = poOwnerFormCenter
  OnShow = FormShow
  ExplicitWidth = 594
  ExplicitHeight = 183
  PixelsPerInch = 96
  TextHeight = 13
  object gpbBotoes: TGroupBox
    Left = 0
    Top = 114
    Width = 588
    Height = 40
    Align = alBottom
    TabOrder = 5
    object btnConfirmar: TBitBtn
      Left = 194
      Top = 5
      Width = 100
      Height = 30
      Caption = '&Confirmar'
      Glyph.Data = {
        
        0000}
      ParentShowHint = False
      ShowHint = False
      TabOrder = 0
      OnClick = btnConfirmarClick
    end
    object btnCancelar: TBitBtn
      Left = 294
      Top = 5
      Width = 100
      Height = 30
      Caption = 'Ca&ncelar'
      Glyph.Data = {
        
        0000}
      ParentShowHint = False
      ShowHint = False
      TabOrder = 1
      OnClick = btnCancelarClick
    end
  end
  object gpbProduto: TGroupBox
    Left = 5
    Top = 5
    Width = 576
    Height = 44
    Caption = 'Produto ( F9 Consulta Produto/Mercadoria )'
    TabOrder = 0
    object btnConsProduto: TSpeedButton
      Left = 538
      Top = 13
      Width = 31
      Height = 26
      Glyph.Data = {
        
        0000}
      OnClick = btnConsProdutoClick
    end
    object dblProduto: TDBLookupComboBox
      Left = 147
      Top = 16
      Width = 387
      Height = 21
      DataField = 'ID_PRODUTO'
      DataSource = dtsProdutoPedido
      KeyField = 'ID_PRODUTO'
      ListField = 'DS_NOME'
      ListSource = dtsProdutos
      TabOrder = 1
      OnClick = dblProdutoClick
    end
    object edtCodigo: TEdit
      Left = 7
      Top = 16
      Width = 134
      Height = 21
      TabOrder = 0
      OnExit = edtCodigoExit
      OnKeyDown = edtCodigoKeyDown
    end
  end
  object gpbValorUnitario: TGroupBox
    Left = 5
    Top = 57
    Width = 130
    Height = 44
    Caption = 'Valor Unit'#225'rio'
    TabOrder = 1
    object dbeValorUnitario: TDBEdit
      Left = 9
      Top = 16
      Width = 112
      Height = 21
      DataField = 'VR_UNITARIO'
      DataSource = dtsProdutoPedido
      TabOrder = 0
      OnExit = dbeValorUnitarioExit
    end
  end
  object gpbQuantidade: TGroupBox
    Left = 174
    Top = 57
    Width = 70
    Height = 44
    Caption = 'Qtde.'
    TabOrder = 2
    object dbeQuantidade: TDBEdit
      Left = 8
      Top = 16
      Width = 54
      Height = 21
      DataField = 'NR_QUANTIDADEPRODUTO'
      DataSource = dtsProdutoPedido
      TabOrder = 0
      OnExit = dbeQuantidadeExit
    end
  end
  object gpbValorTotal: TGroupBox
    Left = 451
    Top = 57
    Width = 130
    Height = 44
    Caption = 'Valor Total'
    TabOrder = 4
    object dbeValorTotal: TDBEdit
      Left = 7
      Top = 16
      Width = 112
      Height = 21
      DataField = 'VR_TOTAL'
      DataSource = dtsProdutoPedido
      TabOrder = 0
    end
  end
  object gpbDesconto: TGroupBox
    Left = 288
    Top = 57
    Width = 130
    Height = 44
    Caption = 'Desconto'
    TabOrder = 3
    object dbeDesconto: TDBEdit
      Left = 8
      Top = 16
      Width = 112
      Height = 21
      DataField = 'VR_DESCONTO'
      DataSource = dtsProdutoPedido
      TabOrder = 0
      OnExit = dbeDescontoExit
    end
  end
  object CamposObrigatorios: TValueListEditor
    Left = 401
    Top = 4
    Width = 129
    Height = 63
    TabStop = False
    DisplayOptions = [doColumnTitles, doKeyColFixed]
    Enabled = False
    KeyOptions = [keyEdit, keyAdd, keyDelete]
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goEditing, goAlwaysShowEditor, goThumbTracking]
    ScrollBars = ssNone
    Strings.Strings = (
      'edtCodigo=Campo Produto n'#227'o informado'
      'dbeValorUnitario='
      '=')
    TabOrder = 6
    TitleCaptions.Strings = (
      'Componente'
      'Mensagem')
    Visible = False
    ColWidths = (
      67
      56)
  end
  object dtsProdutoPedido: TDataSource
    DataSet = frmLctEntradaDetalhe.cdsEntradaItem
    Left = 181
    Top = 58
  end
  object qryProdutos: TIBOQuery
    IB_Connection = dmPrincipal.IB_Connection
    KeyLinks.Strings = (
      'CAD_PRODUTO.ID_PRODUTO')
    PreparedEdits = True
    PreparedInserts = True
    RecordCountAccurate = True
    SQL.Strings = (
      'SELECT CAD_PRODUTO.ID_PRODUTO,'
      '       CAD_PRODUTO.CD_PRODUTO,'
      '       CAD_PRODUTO.DS_NOME,'
      '       CAD_PRODUTO.VR_PRECOCOMPRA'
      'FROM CAD_PRODUTO'
      'ORDER BY CAD_PRODUTO.DS_NOME')
    Left = 115
    Top = 25
  end
  object dtsProdutos: TDataSource
    DataSet = qryProdutos
    Left = 147
    Top = 25
  end
  object qryConsProduto: TIBOQuery
    Params = <
      item
        DataType = ftUnknown
        Name = 'CD_PRODUTO'
        ParamType = ptInput
      end>
    IB_Connection = dmPrincipal.IB_Connection
    KeyLinks.Strings = (
      'CAD_PRODUTO.ID_PRODUTO')
    PreparedEdits = True
    PreparedInserts = True
    RecordCountAccurate = True
    SQL.Strings = (
      'SELECT CAD_PRODUTO.ID_PRODUTO,'
      '       CAD_PRODUTO.CD_PRODUTO,'
      '       CAD_PRODUTO.DS_NOME'
      'FROM CAD_PRODUTO'
      'WHERE CAD_PRODUTO.CD_PRODUTO = :CD_PRODUTO')
    Left = 114
    Top = 58
  end
  object cdsEntradaItemCopy: TClientDataSet
    Active = True
    Aggregates = <>
    Params = <>
    Left = 147
    Top = 58
    Data = {
      
      000000}
    object cdsEntradaItemCopyID_PRODUTO: TIntegerField
      FieldName = 'ID_PRODUTO'
    end
    object cdsEntradaItemCopyVR_UNITARIO: TFloatField
      FieldName = 'VR_UNITARIO'
      DisplayFormat = '#,###,###,##0.00'
    end
    object cdsEntradaItemCopyNR_QUANTIDADEPRODUTO: TFloatField
      FieldName = 'NR_QUANTIDADEPRODUTO'
      DisplayFormat = '#,###,###,##0.000'
    end
    object cdsEntradaItemCopyVR_DESCONTO: TFloatField
      FieldName = 'VR_DESCONTO'
      DisplayFormat = '#,###,###,##0.00'
    end
    object cdsEntradaItemCopyVR_TOTAL: TFloatField
      FieldName = 'VR_TOTAL'
      DisplayFormat = '#,###,###,##0.00'
    end
    object cdsEntradaItemCopyDS_NOME: TStringField
      FieldName = 'DS_NOME'
      Size = 50
    end
  end
end
