inherited frmCadSerial: TfrmCadSerial
  BorderIcons = [biSystemMenu]
  Caption = 'Ativa'#231#227'o do Sistema'
  ClientHeight = 161
  ClientWidth = 397
  Position = poOwnerFormCenter
  ExplicitWidth = 403
  ExplicitHeight = 190
  PixelsPerInch = 96
  TextHeight = 13
  object lblChaveAtivacao: TLabel
    Left = 9
    Top = 6
    Width = 261
    Height = 19
    Caption = 'Digite a chave de ativa'#231#227'o do produto'
    Font.Charset = ANSI_CHARSET
    Font.Color = 14511872
    Font.Height = -16
    Font.Name = 'Calibri'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object gpbBotoes: TGroupBox
    Left = 0
    Top = 121
    Width = 397
    Height = 40
    Align = alBottom
    TabOrder = 1
    object btnConfirmar: TBitBtn
      Left = 99
      Top = 5
      Width = 100
      Height = 30
      Caption = '&Confirmar'
      Glyph.Data = {
        
        0000}
      ParentShowHint = False
      ShowHint = False
      TabOrder = 0
      OnClick = btnConfirmarClick
    end
    object btnCancelar: TBitBtn
      Left = 199
      Top = 5
      Width = 100
      Height = 30
      Caption = 'Ca&ncelar'
      Glyph.Data = {
        
        0000}
      ModalResult = 2
      ParentShowHint = False
      ShowHint = False
      TabOrder = 1
    end
  end
  object GroupBox2: TGroupBox
    Left = 9
    Top = 28
    Width = 378
    Height = 82
    TabOrder = 0
    object lblAparenciaChave: TLabel
      Left = 8
      Top = 13
      Width = 366
      Height = 18
      Caption = 'APAR'#202'NCIA DA CHAVE: XXXXX-XXXXX-XXXXX-XXXXX-XXXXX '
      Font.Charset = ANSI_CHARSET
      Font.Color = -1
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lblChaveProduto: TLabel
      Left = 27
      Top = 52
      Width = 110
      Height = 18
      Caption = 'Chave do produto'
      Font.Charset = ANSI_CHARSET
      Font.Color = -1
      Font.Height = -15
      Font.Name = 'Calibri'
      Font.Style = []
      ParentFont = False
    end
    object edtSerial: TMaskEdit
      Left = 148
      Top = 49
      Width = 201
      Height = 21
      CharCase = ecUpperCase
      EditMask = 'aaaaa-aaaaa-aaaaa-aaaaa-aaaaa;1;_'
      MaxLength = 29
      TabOrder = 0
      Text = '     -     -     -     -     '
    end
  end
  object pnlAguarde: TPanel
    Left = 98
    Top = 36
    Width = 202
    Height = 46
    BevelOuter = bvNone
    BorderStyle = bsSingle
    Caption = 'Aguarde validando serial...'
    Ctl3D = False
    DoubleBuffered = True
    Font.Charset = DEFAULT_CHARSET
    Font.Color = 16744448
    Font.Height = -13
    Font.Name = 'Calibri'
    Font.Style = []
    ParentBackground = False
    ParentCtl3D = False
    ParentDoubleBuffered = False
    ParentFont = False
    TabOrder = 2
    Visible = False
  end
  object CamposObrigatorios: TValueListEditor
    Left = 260
    Top = 8
    Width = 129
    Height = 63
    TabStop = False
    DisplayOptions = [doColumnTitles, doKeyColFixed]
    Enabled = False
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goEditing, goAlwaysShowEditor, goThumbTracking]
    ScrollBars = ssNone
    Strings.Strings = (
      'edtNome=Nome deve ser informado'
      'edtEmail=Email deve ser informado'
      'edtSenha=Senha deve ser informada')
    TabOrder = 3
    TitleCaptions.Strings = (
      'Componente'
      'Mensagem')
    Visible = False
    ColWidths = (
      67
      56)
  end
  object qryInsertSerial: TIBOQuery
    Params = <
      item
        DataType = ftUnknown
        Name = 'NR_SERIAL'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'FG_STATUS'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'DS_EMAIL'
        ParamType = ptInput
      end
      item
        DataType = ftUnknown
        Name = 'DS_CLIENTE'
        ParamType = ptInput
      end>
    IB_Connection = dmPrincipal.IB_Connection
    IB_Transaction = trnsCadSerial
    PreparedEdits = True
    PreparedInserts = True
    RecordCountAccurate = True
    SQL.Strings = (
      
        'INSERT INTO CAD_LIBERACAOSISTEMA(NR_SERIAL, FG_STATUS, DS_EMAIL,' +
        ' DS_CLIENTE) VALUES(:NR_SERIAL, :FG_STATUS, :DS_EMAIL, :DS_CLIEN' +
        'TE)')
    Left = 80
    Top = 56
  end
  object trnsCadSerial: TIBOTransaction
    IB_Connection = dmPrincipal.IB_Connection
    Isolation = tiCommitted
    Left = 112
    Top = 56
  end
  object qryDeleteSerial: TIBOQuery
    IB_Connection = dmPrincipal.IB_Connection
    IB_Transaction = trnsCadSerial
    PreparedEdits = True
    PreparedInserts = True
    RecordCountAccurate = True
    SQL.Strings = (
      'DELETE FROM CAD_LIBERACAOSISTEMA')
    Left = 48
    Top = 56
  end
end
