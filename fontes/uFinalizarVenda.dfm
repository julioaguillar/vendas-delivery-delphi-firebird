inherited frmFinalizarVenda: TfrmFinalizarVenda
  BorderIcons = [biSystemMenu]
  Caption = 'Lan'#231'amento - Sa'#237'da de Produtos/Mercadorias (Finalizar Venda)'
  ClientHeight = 264
  ClientWidth = 619
  Position = poOwnerFormCenter
  OnClose = FormClose
  OnCreate = FormCreate
  ExplicitWidth = 625
  ExplicitHeight = 293
  PixelsPerInch = 96
  TextHeight = 13
  object CamposObrigatorios: TValueListEditor
    Left = 8
    Top = 312
    Width = 245
    Height = 149
    TabStop = False
    DisplayOptions = [doColumnTitles, doKeyColFixed]
    Enabled = False
    KeyOptions = [keyEdit, keyAdd, keyDelete]
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goEditing, goAlwaysShowEditor, goThumbTracking]
    ScrollBars = ssNone
    Strings.Strings = (
      'dbeCliente=O Campo Cliente deve ser informado'
      'dbcFormaPagamento=O Campo Forma de Pagamento deve ser informado'
      'dbeValorPago=O Campo Valor Pago deve ser informado')
    TabOrder = 6
    TitleCaptions.Strings = (
      'Componente'
      'Mensagem')
    Visible = False
    ColWidths = (
      94
      136)
  end
  object gpbCliente: TGroupBox
    Left = 11
    Top = 8
    Width = 598
    Height = 81
    Caption = 'CLIENTE (F9 PARA CONSULTAR)'
    TabOrder = 0
    object gpbCodigo: TGroupBox
      Left = 11
      Top = 22
      Width = 166
      Height = 50
      Caption = 'C'#211'DIGO'
      TabOrder = 0
      object dbeCliente: TDBEdit
        Left = 9
        Top = 17
        Width = 148
        Height = 24
        DataField = 'CD_CLIENTE'
        DataSource = dtsVenda
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
        OnExit = dbeClienteExit
        OnKeyDown = dbeClienteKeyDown
      end
    end
    object gpbNome: TGroupBox
      Left = 183
      Top = 22
      Width = 405
      Height = 50
      Caption = 'NOME'
      Enabled = False
      TabOrder = 1
      object dbeNomeCliente: TDBEdit
        Left = 9
        Top = 17
        Width = 388
        Height = 24
        DataField = 'DS_NOME'
        DataSource = dtsConsCliente
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -13
        Font.Name = 'Tahoma'
        Font.Style = [fsBold]
        ParentFont = False
        TabOrder = 0
      end
    end
  end
  object gpbFormaPagamento: TGroupBox
    Left = 11
    Top = 100
    Width = 235
    Height = 50
    Caption = 'FORMA DE PAGAMENTO'
    TabOrder = 1
    object dbcFormaPagamento: TDBComboBox
      Left = 8
      Top = 18
      Width = 218
      Height = 21
      Style = csDropDownList
      BevelInner = bvNone
      Ctl3D = True
      DataField = 'DS_FORMAPAGAMENTO'
      DataSource = dtsVenda
      Items.Strings = (
        'DINHEIRO'
        'CHEQUE'
        'CARTAO CREDITO'
        'CARTAO DEBITO'
        'OUTROS')
      ParentCtl3D = False
      TabOrder = 0
      OnChange = dbcFormaPagamentoChange
    end
  end
  object gpbValorVenda: TGroupBox
    Left = 252
    Top = 100
    Width = 176
    Height = 50
    Caption = 'VALOR TOTAL VENDA'
    Enabled = False
    TabOrder = 2
    object dbtTotal: TDBText
      Left = 9
      Top = 19
      Width = 155
      Height = 18
      Alignment = taRightJustify
      DataField = 'VR_TOTALVENDA'
      DataSource = dtsVenda
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object gpbValorPago: TGroupBox
    Left = 434
    Top = 100
    Width = 175
    Height = 50
    Caption = 'VALOR PAGO'
    TabOrder = 3
    object dbeValorPago: TDBEdit
      Left = 9
      Top = 17
      Width = 156
      Height = 24
      DataField = 'VR_TOTALPAGO'
      DataSource = dtsVenda
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -13
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 0
    end
  end
  object gpbTroco: TGroupBox
    Left = 434
    Top = 160
    Width = 175
    Height = 50
    Caption = 'TROCO'
    Enabled = False
    TabOrder = 4
    object dbtTroco: TDBText
      Left = 9
      Top = 15
      Width = 156
      Height = 29
      Alignment = taCenter
      DataField = 'VR_TROCO'
      DataSource = dtsVenda
      Font.Charset = ANSI_CHARSET
      Font.Color = 11361544
      Font.Height = -21
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
    end
  end
  object gpbBotoes: TGroupBox
    Left = 0
    Top = 222
    Width = 619
    Height = 42
    Align = alBottom
    TabOrder = 5
    object btnConfirmar: TBitBtn
      Left = 208
      Top = 6
      Width = 100
      Height = 30
      Caption = '&Finalizar'
      Glyph.Data = {
        
        0000}
      ParentShowHint = False
      ShowHint = False
      TabOrder = 0
      OnClick = btnConfirmarClick
    end
    object btnCancelar: TBitBtn
      Left = 311
      Top = 6
      Width = 100
      Height = 30
      Caption = 'Ca&ncelar'
      Glyph.Data = {
        
        0000}
      ParentShowHint = False
      ShowHint = False
      TabOrder = 1
      OnClick = btnCancelarClick
    end
  end
  object dtsVenda: TDataSource
    DataSet = frmLctVenda.cdsVenda
    Left = 56
    Top = 144
  end
  object qryConsCliente: TIBOQuery
    Params = <
      item
        DataType = ftUnknown
        Name = 'CD_CLIENTE'
        ParamType = ptInput
      end>
    IB_Connection = dmPrincipal.IB_Connection
    PreparedEdits = True
    PreparedInserts = True
    RecordCountAccurate = True
    SQL.Strings = (
      'SELECT CAD_CLIENTE.CD_CLIENTE,'
      '       CAD_CLIENTE.DS_NOME'
      'FROM CAD_CLIENTE'
      'WHERE CAD_CLIENTE.CD_CLIENTE = :CD_CLIENTE')
    Left = 24
    Top = 176
  end
  object dtsConsCliente: TDataSource
    DataSet = qryConsCliente
    Left = 24
    Top = 144
  end
end
