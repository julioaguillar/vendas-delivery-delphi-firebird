inherited frmCadProdutoPedido: TfrmCadProdutoPedido
  BorderIcons = [biSystemMenu]
  Caption = 'Lan'#231'amento - Or'#231'amento (Produto/Mercadoria)'
  ClientHeight = 156
  ClientWidth = 620
  Position = poOwnerFormCenter
  OnShow = FormShow
  ExplicitWidth = 626
  ExplicitHeight = 185
  PixelsPerInch = 96
  TextHeight = 13
  object gpbBotoes: TGroupBox
    Left = 0
    Top = 116
    Width = 620
    Height = 40
    Align = alBottom
    TabOrder = 6
    ExplicitTop = 114
    ExplicitWidth = 588
    object btnConfirmar: TBitBtn
      Left = 210
      Top = 5
      Width = 100
      Height = 30
      Caption = '&Confirmar'
      Glyph.Data = {
        
        0000}
      ParentShowHint = False
      ShowHint = False
      TabOrder = 0
      OnClick = btnConfirmarClick
    end
    object btnCancelar: TBitBtn
      Left = 310
      Top = 5
      Width = 100
      Height = 30
      Caption = 'Ca&ncelar'
      Glyph.Data = {
        
        0000}
      ParentShowHint = False
      ShowHint = False
      TabOrder = 1
      OnClick = btnCancelarClick
    end
  end
  object gpbProduto: TGroupBox
    Left = 11
    Top = 9
    Width = 597
    Height = 44
    Caption = 'Produto ( F9 Consulta Produto/Mercadoria )'
    TabOrder = 0
    object btnConsProduto: TSpeedButton
      Left = 558
      Top = 13
      Width = 31
      Height = 26
      Glyph.Data = {
        
        0000}
      OnClick = btnConsProdutoClick
    end
    object dblProduto: TDBLookupComboBox
      Left = 158
      Top = 16
      Width = 397
      Height = 21
      DataField = 'ID_PRODUTO'
      DataSource = dtsProdutoPedido
      KeyField = 'ID_PRODUTO'
      ListField = 'DS_NOME'
      ListSource = dtsProdutos
      TabOrder = 1
      OnClick = dblProdutoClick
    end
    object edtCodigo: TEdit
      Left = 7
      Top = 16
      Width = 146
      Height = 21
      TabOrder = 0
      OnExit = edtCodigoExit
      OnKeyDown = edtCodigoKeyDown
    end
  end
  object gpbValorUnitario: TGroupBox
    Left = 11
    Top = 61
    Width = 130
    Height = 44
    Caption = 'Valor Unit'#225'rio'
    TabOrder = 1
    object dbeValorUnitario: TDBEdit
      Left = 9
      Top = 16
      Width = 112
      Height = 21
      DataField = 'VR_UNITARIO'
      DataSource = dtsProdutoPedido
      TabOrder = 0
      OnExit = dbeValorUnitarioExit
    end
  end
  object gpbQuantidade: TGroupBox
    Left = 145
    Top = 61
    Width = 70
    Height = 44
    Caption = 'Qtde.'
    TabOrder = 2
    object dbeQuantidade: TDBEdit
      Left = 8
      Top = 16
      Width = 54
      Height = 21
      DataField = 'NR_QUANTIDADEPRODUTO'
      DataSource = dtsProdutoPedido
      TabOrder = 0
      OnExit = dbeQuantidadeExit
    end
  end
  object gpbValorTotal: TGroupBox
    Left = 481
    Top = 61
    Width = 127
    Height = 44
    Caption = 'Valor Total'
    TabOrder = 5
    object dbeValorTotal: TDBEdit
      Left = 7
      Top = 16
      Width = 112
      Height = 21
      DataField = 'VR_TOTAL'
      DataSource = dtsProdutoPedido
      TabOrder = 0
    end
  end
  object gpbDesconto: TGroupBox
    Left = 350
    Top = 61
    Width = 127
    Height = 44
    Caption = 'Desconto'
    TabOrder = 4
    object dbeDesconto: TDBEdit
      Left = 8
      Top = 16
      Width = 112
      Height = 21
      DataField = 'VR_DESCONTO'
      DataSource = dtsProdutoPedido
      TabOrder = 0
      OnExit = dbeDescontoExit
    end
  end
  object CamposObrigatorios: TValueListEditor
    Left = 324
    Top = 8
    Width = 288
    Height = 95
    TabStop = False
    DisplayOptions = [doColumnTitles, doKeyColFixed]
    Enabled = False
    KeyOptions = [keyEdit, keyAdd, keyDelete]
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goEditing, goAlwaysShowEditor, goThumbTracking]
    ScrollBars = ssNone
    Strings.Strings = (
      'edtCodigo=Campo Produto n'#227'o informado')
    TabOrder = 7
    TitleCaptions.Strings = (
      'Componente'
      'Mensagem')
    Visible = False
    ColWidths = (
      92
      124)
  end
  object gpbAcrescimo: TGroupBox
    Left = 219
    Top = 61
    Width = 127
    Height = 44
    Caption = 'Acrescimo'
    TabOrder = 3
    object dbeAcrescimo: TDBEdit
      Left = 8
      Top = 16
      Width = 112
      Height = 21
      DataField = 'VR_ACRESCIMO'
      DataSource = dtsProdutoPedido
      TabOrder = 0
      OnExit = dbeAcrescimoExit
    end
  end
  object dtsProdutoPedido: TDataSource
    DataSet = frmLctPedidoDetalhe.cdsPedidoItem
    Left = 301
    Top = 58
  end
  object qryProdutos: TIBOQuery
    IB_Connection = dmPrincipal.IB_Connection
    KeyLinks.Strings = (
      'CAD_PRODUTO.ID_PRODUTO')
    PreparedEdits = True
    PreparedInserts = True
    RecordCountAccurate = True
    SQL.Strings = (
      'SELECT CAD_PRODUTO.ID_PRODUTO,'
      '       CAD_PRODUTO.CD_PRODUTO,'
      '       CAD_PRODUTO.DS_NOME,'
      '       CAD_PRODUTO.VR_PRECOCOMPRA,'
      '       CAD_PRODUTO.VR_PRECOVENDA01'
      'FROM CAD_PRODUTO'
      'ORDER BY CAD_PRODUTO.DS_NOME')
    Left = 235
    Top = 25
  end
  object dtsProdutos: TDataSource
    DataSet = qryProdutos
    Left = 267
    Top = 25
  end
  object qryConsProduto: TIBOQuery
    Params = <
      item
        DataType = ftUnknown
        Name = 'CD_PRODUTO'
        ParamType = ptInput
      end>
    IB_Connection = dmPrincipal.IB_Connection
    KeyLinks.Strings = (
      'CAD_PRODUTO.ID_PRODUTO')
    PreparedEdits = True
    PreparedInserts = True
    RecordCountAccurate = True
    SQL.Strings = (
      'SELECT CAD_PRODUTO.ID_PRODUTO,'
      '       CAD_PRODUTO.CD_PRODUTO,'
      '       CAD_PRODUTO.DS_NOME'
      'FROM CAD_PRODUTO'
      'WHERE CAD_PRODUTO.CD_PRODUTO = :CD_PRODUTO')
    Left = 234
    Top = 58
  end
  object cdsPedidoItemCopy: TClientDataSet
    Active = True
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'ID_PRODUTO'
        DataType = ftInteger
      end
      item
        Name = 'VR_UNITARIO'
        DataType = ftFloat
      end
      item
        Name = 'NR_QUANTIDADEPRODUTO'
        DataType = ftFloat
      end
      item
        Name = 'VR_ACRESCIMO'
        DataType = ftFloat
      end
      item
        Name = 'VR_DESCONTO'
        DataType = ftFloat
      end
      item
        Name = 'VR_TOTAL'
        DataType = ftFloat
      end
      item
        Name = 'DS_NOME'
        DataType = ftString
        Size = 50
      end>
    IndexDefs = <>
    Params = <>
    StoreDefs = True
    Left = 267
    Top = 58
    Data = {
      
      4D4501004900000001000557494454480200020032000000}
    object cdsPedidoItemCopyID_PRODUTO: TIntegerField
      FieldName = 'ID_PRODUTO'
    end
    object cdsPedidoItemCopyVR_UNITARIO: TFloatField
      FieldName = 'VR_UNITARIO'
      DisplayFormat = ',0.00'
    end
    object cdsPedidoItemCopyNR_QUANTIDADEPRODUTO: TFloatField
      FieldName = 'NR_QUANTIDADEPRODUTO'
      DisplayFormat = ',0.000'
    end
    object cdsPedidoItemCopyVR_ACRESCIMO: TFloatField
      FieldName = 'VR_ACRESCIMO'
      DisplayFormat = ',0.00'
    end
    object cdsPedidoItemCopyVR_DESCONTO: TFloatField
      FieldName = 'VR_DESCONTO'
      DisplayFormat = ',0.00'
    end
    object cdsPedidoItemCopyVR_TOTAL: TFloatField
      FieldName = 'VR_TOTAL'
      DisplayFormat = ',0.00'
    end
    object cdsPedidoItemCopyDS_NOME: TStringField
      FieldName = 'DS_NOME'
      Size = 50
    end
  end
end
